...
int read_header(server_params *params, request *req) {
    ...
    if (req->status == ONE_LF) {
      *req->header_end = '\0';
      ...
      if (req->logline) {
        ...
        if (process_logline(req) == 0) return 0;
  ...
  if (req->status < BODY_READ) {
    ...
    if (bytes < 0) {
      if (bytes == BOA_E_INTR) return 1;
      if (bytes == BOA_E_AGAIN) return -1;
...
int read_body(request *req) {
  ...
  if (bytes_to_read > bytes_free) bytes_to_read = bytes_free;
...
int write_body(request *req) {
  ...
  if (bytes_to_write == 0) { /* nothing left in buffer to write */
    ...
    if (req->filepos >= req->filesize) return init_cgi(req);
