...
int modified_since(time_t mtime, char *if_modified_since) {
  ...
  while (*ims_info != ' ' && *ims_info != '\0') ++ims_info;
  if (*ims_info != ' ') return -1;
  ...
  if ((comp = 1900 + file_gmt->tm_year - year)) return (comp > 0);
  if ((comp = file_gmt->tm_mon - month)) return (comp > 0);
  if ((comp = file_gmt->tm_mday - day)) return (comp > 0);
  if ((comp = file_gmt->tm_hour - hour)) return (comp > 0);
  if ((comp = file_gmt->tm_min - minute)) return (comp > 0);
  if ((comp = file_gmt->tm_sec - second)) return (comp > 0);
...
int unescape_uri(char *uri, char **query_string) {
  ...
  while ((c = *uri_old)) {
    if (c == '%') {
      ...
      if (query_string) *query_string = ++uri_old;
      /* stop here */
      *uri = '\0';
      return (1);
      break;
...
void close_tmp_fd(tmp_fd *fds) {
  if (fds->pipe) {
    if (fds->fds[1] != -1) close(fds->fds[1]);
  }

  if (fds->fds[0] != -1) close(fds->fds[0]);
...
void create_url(char *buffer, int buffer_size, int secure, const char *hostname,
  ...
  if (secure) {
    proto = "https";
    if (port != 443) do_port = 1;
  } else {
    proto = "http";
    if (port != 80) do_port = 1;
  }
