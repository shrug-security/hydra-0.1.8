...
/* NCSA added */
  ...
  for (i = 0; i < ix; ++i) {
    if (common_cgi_env[i] == NULL) {
      log_error_time();
      fprintf(stderr,
              "Unable to allocate a component of common_cgi_env - out of "
              "memory.\n");
      exit(1);
    }
  }
...
int add_cgi_env(request *req, const char *key, const char *value,
  ...
  if (req->cgi_env_index < CGI_ENV_MAX) {
    ...
    if (prefix_len) memcpy(p, "HTTP_", 5);
...
int complete_env(request *req) {
  ...
  for (i = 0; common_cgi_env[i]; i++) req->cgi_env[i] = common_cgi_env[i];

  w = hydra_method_str(req->method);
  my_add_cgi_env(req, "REQUEST_METHOD", w);

  if (req->action) my_add_cgi_env(req, "REDIRECT_STATUS", "200");
  ...
  if (req->path_info) my_add_cgi_env(req, "PATH_INFO", req->path_info);
  ...
  if (req->query_string) my_add_cgi_env(req, "QUERY_STRING", req->query_string);
...
#ifdef ACCEPT_ON
  if (req->accept[0]) my_add_cgi_env(req, "HTTP_ACCEPT", req->accept);
#endif
  ...
  log_error_time();
  fprintf(stderr,
          "Not enough space in CGI environment for remainder"
          " of variables.\n");
  return 0;
}
