...
static unsigned sdbm_hash(const char *str) {
  ...
  while ((c = *str++) && count--) hash = c + (hash << 6) + (hash << 16) - hash;
...
static hash_struct *hash_insert(hash_struct *table[], unsigned int hash,
  ...
  if (trailer->key == NULL || trailer->value == NULL) {
    free(trailer);
    if (trailer->key) free(trailer->key);
    if (trailer->value) free(trailer->value);
...
char *get_mime_type(const char *filename) {
   ...
   */
  if (!extension || extension[1] == '\0') return default_type;
