...
virthost *find_virthost(const char *_host, int hostlen) {
  ...
  if (hostlen == 0) hostlen = strlen(_host);

  if (hostlen >= MAX_SITENAME_LENGTH) return NULL;
  ...
  for (i = 0; i < VIRTHOST_HASHTABLE_SIZE; ++i) { /* these limits OK? */
    if (virthost_hashtable[i]) {
      ...
      while (temp) {
        ...
        if (temp->host) free(temp->host);
        free(temp->access_nodes);
        if (temp->ip) free(temp->ip);
        if (temp->document_root) free(temp->document_root);
        if (temp->user_dir) free(temp->user_dir);
