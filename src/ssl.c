...
int initialize_ssl(void) {
  ...
  if (ssl_session_cache != 0) wrap_db_init();
   ...
   */
  i = 0;
  if (parse_cs_string(ssl_kx, "RSA") != 0) kx_priority[i++] = GNUTLS_KX_RSA;
   ...
   */
  i = 0;
  if (parse_cs_string(ssl_mac, "MD5") != 0) mac_priority[i++] = GNUTLS_MAC_MD5;
  if (parse_cs_string(ssl_mac, "SHA1") != 0) mac_priority[i++] = GNUTLS_MAC_SHA;
...
static int wrap_db_store(void *dbf, gnutls_datum key, gnutls_datum data) {
  ...
  if (key.size > SESSION_ID_SIZE) return -1;
  if (data.size > SESSION_DATA_SIZE) return -1;
    ...
    if (ssl_verify >= 1) {
      ...
      if (cert_list == NULL) {
        ...
        if (valid || verify & GNUTLS_CERT_INVALID ||
          ...
          if (verify & GNUTLS_CERT_REVOKED) fprintf(stderr, ", Revoked");
...
int initialize_ssl(void) {
  log_error_time();
  fprintf(stderr,
          "tls: SSL is not available in this build. Disable SSL in Hydra's "
          "configuration file.\n");
  exit(1);
}
