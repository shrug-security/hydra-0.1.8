...
ssize_t socket_recv(request* req, void* buf, size_t buf_size) {
  ...
  if (req->secure) {
    ...
    if (bytes < 0) {
      if (bytes == GNUTLS_E_INTERRUPTED) return BOA_E_INTR;
    ...
    if (bytes == -1) {
      if (errno == EINTR) return BOA_E_INTR;
...
ssize_t socket_send(request* req, const void* buf, size_t buf_size) {
  ...
  if (req->secure) {
    ...
    if (bytes == -1) {
      if (errno == EINTR) return BOA_E_INTR;
      if (errno == EPIPE) return BOA_E_PIPE;
      if (errno == EAGAIN || errno == EWOULDBLOCK) /* request blocked */
        return BOA_E_AGAIN;
...
void socket_flush(int fd) {
   ...
   */
  if (setsockopt(fd, IPPROTO_TCP, TCP_CORK, (void*)&zero, sizeof(zero)) == -1) {
