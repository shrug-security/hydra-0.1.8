...
inline static void decrease_global_total_connections(int ssl) {
   ...
   */
  if (max_connections == INT_MAX && max_ssl_connections == INT_MAX) return;
...
void get_request(server_params *params, socket_type *server_s) {
  ...
  if (fd == -1) {
    if (errno != EAGAIN && errno != EWOULDBLOCK) /* abnormal error */
      WARN("accept");
    else
      /* no requests */
      server_s->pending_requests = 0;
...
static void free_request(server_params *params, request **list_head_addr,
  ...
  if (req->mmap_entry_var) release_mmap(req->mmap_entry_var);
  /* FIXME: Why is it needed? */
  else if (req->data_mem)
    munmap(req->data_mem, req->filesize);

  if (req->data_fd != -1) close(req->data_fd);

  close_tmp_fd(&req->post_data_fd);

  if (req->response_status >= 400) params->status.errors++;
...
void process_requests(server_params *params, socket_type *server_s) {
  ...
  while (current) {
    ...
    if (params->sigterm_flag) SQUASH_KA(current);
     ...
     */
    if (server_s->pending_requests) get_request(params, server_s);
...
int process_header_end(server_params *params, request *req) {
  ...
  if (req->method == M_POST) {
    ...
    if (req->post_data_fd.fds[0] == -1) return (0);
  ...
  *val1 = *val2 = 0;
  ...
  if (value[0] == '-') { /* Handle case "bytes=-1024" */
    ...
    *val2 = boa_atoll(&value[1]);
    ...
    if ((len = strlen(value)) >= sizeof(buf)) return -1;
  ...
  switch (line[0]) {
    ...
    case 'H':
      if (!memcmp(line, "HOST", 4)) {
        ...
        if (!add_cgi_env(req, "HOST", value, 1)) return 0;
      } else
        goto just_add_header;
      break;
    ...
    case 'I':
      if (!memcmp(line, "IF_", 3)) {
        ...
        if (!add_cgi_env(req, line, value, 1)) return 0;
        break;
      } else
        goto just_add_header;
    ...
    case 'R':
      ...
      if (!memcmp(line, "REFERER", 8)) {
        ...
        if (!add_cgi_env(req, "REFERER", value, 1)) return 0;
    ...
    case 'U':
      if (!memcmp(line, "USER_AGENT", 11)) {
        ...
        if (!add_cgi_env(req, "USER_AGENT", value, 1)) return 0;
      } else
        goto just_add_header;
      break;
...
void add_accept_header(request *req, char *mime_type) {
  ...
  if ((l + l2 + 2) >= MAX_HEADER_LENGTH) return;
