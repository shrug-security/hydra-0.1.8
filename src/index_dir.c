...
char *http_escape_string(char *inp, char *buf, const int len) {
  ...
  if (buf == NULL && max) buf = malloc(sizeof(unsigned char) * (max + 1));
...
int index_directory(char *dir, char *title) {
  ...
  for (i = 0; i < numdir; ++i) {
    ...
    if (stat(dirbuf->d_name, &statbuf) == -1) continue;
...
int main(int argc, char *argv[]) {
  ...
  printf(
      "</table>\n<hr noshade>\nIndex generated %s %s\n"
      "<!-- This program is part of the Boa Webserver Copyright (C) 1991-2002 "
      "http://www.boa.org -->\n"
      "</body>\n</html>\n",
