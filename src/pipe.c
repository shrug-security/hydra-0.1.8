...
int write_from_pipe(request *req) {
  ...
  if (bytes_to_write == 0) {
    if (req->cgi_status == CGI_DONE) return 0;
...
int io_shuffle(request *req) {
  ...
  if (foo > system_bufsize) foo = system_bufsize;
...
retrysendfile:
  ...
  if (foo >= 0) {
    if (req->filepos >= req->pipe_range_stop) return 0;
    return 1;
  } else {
...
int io_shuffle(request *req) {
  ...
  restartread:
    ...
    if (bytes_read == -1) {
      if (errno == EINTR)
        ...
        if (req->buffer_end - req->buffer_start == 0) return -1;
      } else {
        req->status = DEAD;
        log_error_doc(req);
  ...
  if (bytes_to_write == 0) {
    if (req->data_fd == 0) return 0; /* done */
