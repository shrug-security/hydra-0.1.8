...
#define GNUTLS_MAX_SESSION_ID 32
  ...
  if (!add_cgi_env(req, "HTTPS", "on", 0)) return 0;
...

  {
    ...
    if (!add_cgi_env(req, "SSL_VERSION_LIBRARY", version, 0)) return 0;
  }
...

  {
    ...
    if (!add_cgi_env(req, "SSL_CIPHER_ALGKEYSIZE", buf, 0)) return 0;

    if (!add_cgi_env(req, "SSL_CIPHER_USEKEYSIZE", buf, 0)) return 0;
    ...
    if (!add_cgi_env(req, "SSL_CIPHER_EXPORT", p, 0)) return 0;
  }
  ...
  if (gnutls_session_get_id(req->ssl_state, session_id, &session_id_length) >=
    ...
    *p = 0;
    ...
    if (!add_cgi_env(req, "SSL_SESSION_ID", str_session_id, 0)) return 0;
  }
